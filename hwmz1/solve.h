...
// string solve(string maze){
    // Step 1: Parse the maze into a 2D grid and find 'S' (start) and 'E' (end)
    // grid, start, end = parseMaze(maze)
    ...
    // // Step 5: Mark the path with 'o's in the maze
    // solution = maze string (split into lines)
    // FOR each (row, col) in path (excluding start and end):
    //     solution[row][col] = 'o'

    // RETURN solution as a single string with newlines

// };

// string path(Vertex* start, Vertex* end){
//     Vertex* curr = d;
...
string solve(string maze) {
    ...
    // We'll try each entrance until we find a solution
    for (auto [start_row, start_col] : entrances) {
        ...
        while (!q.empty() && !exit_found) {
            ...
            // Check if current position is an exit (different from start)
